...
- 订阅更新：支持订阅配置的自动更新、立即更新、托盘一键更新全部订阅，并在更新成功/失败时发送系统通知。
- 配置清空：一键清空配置并恢复默认配置。
- 系统代理切换（Windows）。
...
- 每个配置的订阅链接分别存储，互不覆盖。
- 默认存储在系统密钥链，配置文件保留回退副本用于密钥链丢失时恢复。
- Windows 查看/删除路径：控制面板 → 凭据管理器 → Windows 凭据，目标项名称为 `Mihomo-Despicable-Infiltrator`，账号名为 `subscription:{profile}`。

## 截图

- TODO: 补全托盘菜单与订阅更新通知截图。
