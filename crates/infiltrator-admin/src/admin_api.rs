...
use self::handlers::proxies::{
    get_proxies_http, list_runtime_proxy_delays_http, select_proxy_http, set_proxy_mode_http,
    test_all_runtime_proxy_delays_http, test_runtime_proxy_delay_http,
};
...
pub fn router<C: AdminApiContext>(state: AdminApiState<C>) -> Router {
    Router::new()
        ...
        .route("/admin/api/doctor", get(run_doctor_http::<C>))
        .route(
            "/admin/api/doctor/checks",
            get(list_doctor_checks_http::<C>),
        )
        .route(
            "/admin/api/doctor/checks/{id}",
            get(explain_doctor_check_http::<C>),
        )
        .route("/admin/api/doctor/fix", post(fix_doctor_http::<C>))
        .route("/admin/api/bootstrap", post(run_bootstrap_http::<C>))
