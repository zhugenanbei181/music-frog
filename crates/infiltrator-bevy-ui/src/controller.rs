//! The live mihomo controller pump (BEVY-005's real data seam).
//!
//! Architecture — a background thread, never a blocking bevy system:
//!
//! 1. [`MihomoOverviewSource::spawn`] starts one [`std::thread`] that owns
//!    a current-thread tokio runtime and a
//!    [`mihomo_client`](mihomo_api::client::MihomoClient) against the
//!    configured external controller. The thread samples the core on a
//!    fixed interval (`/connections` every tick — totals deltas become the
//!    up/down rates; `/memory` and `/configs` every few ticks), folds the
//!    responses into an [`OverviewProjection`] and publishes it through a
//!    bounded (`PUMP_CHANNEL_CAPACITY`) channel.
//! 2. [`PumpDrainPlugin`] installs the bevy-side resource bridge
//!    ([`OverviewPumpBridge`]) plus a per-frame drain system: it
//!    `try_recv`s every queued snapshot each frame and triggers
//!    `OverviewProjectionUpdated` with the newest one — the page's
//!    existing in-place refresh path, untouched. The drain also enforces
//!    the failure-verdict dwell ([`FailureDwell`]): a visible failure
//!    (refused mode switch, dropped channel, dead core) stays mounted for
//!    at least [`MIN_FAILURE_DWELL`] *and* until the next successful
//!    sample — whichever is later — instead of being washed away by the
//!    next routine tick.
//! 3. Mode commands ride the [`OverviewSource`] trait: `set_mode` posts
//!    into the pump's command channel, the pump thread executes
//!    `PATCH /configs {"mode": …}` on its own runtime and answers the
//!    typed receipt. The shell's in-flight latch and ack drain live in
//!    [`crate::app`].
//!
//! Cancellation is structural ("Drop 即停"): the pump thread holds the
//! receiving ends only, so when the last [`MihomoOverviewSource`] clone
//! drops, the command channel disconnects and the thread exits; a dropped
//! bridge disconnects the snapshot channel and the thread exits the same
//! way. No flags, no joins, no leaks.
//!
//! Configuration seam (capture-env style, read once by the launcher in
//! [`crate::run`]): `INFILTRATOR_BEVY_CONTROLLER` (e.g.
//! `http://127.0.0.1:9099`) switches the frontend from the demo fixture to
//! this pump; `INFILTRATOR_BEVY_SECRET` carries the controller's bearer
//! secret. Unset env keeps the demo frontend bit-for-bit (zero capture
//! regression); a configured but unreachable controller projects
//! [`OverviewState::Unavailable`] with a typed failure reason — a visible
