...
use super::*;
    ...
    fn test_trace_28_plus_rule_matrix() {
        let rules = vec![
            RuleEntry { rule: "SRC-IP-CIDR,10.0.0.0/8,INTERNAL".into(), enabled: true },
            RuleEntry { rule: "SRC-PORT,5000-6000,SPECIAL_SRC".into(), enabled: true },
            RuleEntry { rule: "IN-PORT,7895,CLASH_IN".into(), enabled: true },
            RuleEntry { rule: "IN-TYPE,TUN,TUN_PROXY".into(), enabled: true },
            RuleEntry { rule: "PROCESS-PATH,/usr/bin/curl,CURL_DIRECT".into(), enabled: true },
            RuleEntry { rule: "PROCESS-NAME,steam.exe,GAME".into(), enabled: true },
            RuleEntry { rule: "NETWORK,udp,UDP_REJECT".into(), enabled: true },
            RuleEntry { rule: "DSCP,46,VOIP_HIGH".into(), enabled: true },
            RuleEntry { rule: "UID,1001,USER_PROXY".into(), enabled: true },
            RuleEntry { rule: "PACKAGE-NAME,com.google.android.youtube,YT_APP".into(), enabled: true },
            RuleEntry { rule: "MATCH,DIRECT".into(), enabled: true },
        ];
        ...
        assert_eq!(trace_rules(&rules, &ctx_intype).unwrap().target, "TUN_PROXY");

        // 5. PROCESS-PATH
        let ctx_ppath = TrafficContext::new().with_process_path("/usr/bin/curl");
        assert_eq!(trace_rules(&rules, &ctx_ppath).unwrap().target, "CURL_DIRECT");
    ...
    fn test_decision_chain_generation() {
        ...
        let parsed = matched.as_ref().and_then(|m| parse_rule_str(&rules[m.index].rule).ok());
