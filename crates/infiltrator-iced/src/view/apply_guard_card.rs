...
pub fn apply_guard_card<'a>(state: &'a AppState, lang: &Lang<'_>) -> Element<'a, Message> {
    ...
    let (stage_label, stage_badge_kind): (String, BadgeKind) = match &guard.stage {
        ...
        ApplyTransactionStage::Preflight => (
            lang.tr("apply_guard_stage_preflight").to_string(),
            BadgeKind::Warning,
        ),
        ApplyTransactionStage::Reloading => (
            lang.tr("apply_guard_stage_reloading").to_string(),
            BadgeKind::Warning,
        ),
        ApplyTransactionStage::Probing => (
            lang.tr("apply_guard_stage_probing").to_string(),
            BadgeKind::Accent,
        ),
        ApplyTransactionStage::Committed => (
            lang.tr("apply_guard_status_committed").to_string(),
            BadgeKind::Success,
        ),
        ApplyTransactionStage::RolledBack(_) => (
            lang.tr("apply_guard_status_rolled_back").to_string(),
            BadgeKind::Danger,
        ),
    };
    ...
    card(
        ...
        column![
            text(lang.tr("apply_guard_desc").to_string())
                .size(12)
                .style(|t: &Theme| text::Style {
                    color: Some(tokens(t).text_secondary)
                }),
