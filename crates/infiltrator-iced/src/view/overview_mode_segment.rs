...
pub fn overview_mode_segment<'a>(state: &AppState, lang: &Lang<'a>) -> Element<'a, Message> {
    ...
    let mut pills_row = row![].spacing(theme::SP_SM).align_y(Alignment::Center).width(Length::Fill);
    ...
    for mode in ProxyMode::ALL {
        ...
        let btn_content = text(label)
            .size(13)
            .font(if is_current { FONT_SEMIBOLD } else { FONT_MEDIUM });
            ...
            .style(move |t: &Theme, _status| {
                ...
                button::Style {
                    ...
                    border: Border {
                        ...
                        width: 1.0,
                        color: if is_current { tk.accent } else { Color::TRANSPARENT },
                    },
                    ..Default::default()
                }
            });
        ...
        let interactive_btn: Element<'a, Message> = if is_selectable && !is_current {
            btn.on_press(Message::SetProxyMode(mode.to_wire().to_owned())).into()
        } else {
            btn.into()
        };
    ...
    let header = row![
        ...
        text(format!("当前：{}", match current_mode {
            ProxyMode::Rule => lang.tr("mode_rule").into_owned(),
            ProxyMode::Global => lang.tr("mode_global").into_owned(),
            ProxyMode::Direct => lang.tr("mode_direct").into_owned(),
            ProxyMode::Script => lang.tr("mode_script").into_owned(),
    ...
    container(
        column![header, Space::new().height(theme::SP_XS), pills_row].spacing(theme::SP_SM),
    )
    .width(Length::Fill)
    .padding(theme::SP_LG)
    .style(card_surface)
