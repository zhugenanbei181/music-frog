    ...
    for binding in &state.shell.hotkeys_config {
        ...
        let row_item = row![
            ...
            text_input("e.g. Ctrl+Alt+P", &binding.combo)
                .on_input(move |combo| Message::UpdateHotkeyCombo {
                    id: binding_id.clone(),
                    combo
                })
                .padding([4, 8])
...
pub fn view(state: &AppState) -> Element<'_, Message> {
    ...
    let system_section =
        system_integration_card(state, &lang, is_en, theme_selector, selected_language);
