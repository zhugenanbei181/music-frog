...
pub(super) fn translate_zh_cn_ext(key: &str) -> Cow<'static, str> {
    match key {
        ...
        "dns_latency_run" => "测速 (Probe)".into(),
        "dns_latency_probing" => "测速中…".into(),
        "dns_latency_not_probed_yet" => "本次会话尚未测速".into(),
        "dns_latency_all_measured" => "全部 {count} 个上游均应答 ({best_ms}-{worst_ms} ms)".into(),
        "dns_latency_partial" => "{total} 个上游中仅 {measured} 个应答".into(),
        "dns_latency_none_reachable" => "{total} 个上游全部无应答，解析不可达".into(),
        "dns_latency_primary" => "主上游".into(),
        "dns_latency_fallback" => "Fallback".into(),
        "dns_latency_measured" => "{ms} ms".into(),
        "dns_latency_timeout" => "超时无应答".into(),
        "dns_latency_invalid_response" => "应答未通过校验: {reason}".into(),
        "dns_latency_failed" => "探测失败: {reason}".into(),
        "dns_latency_not_probed" => "未探测: {reason}".into(),
        "dns_self_heal_title" => "DNS 故障自愈检测".into(),
        "dns_self_heal_desc" => "合并 dns.listen 端口占用、上游可达性与拓扑审计的共享观测".into(),
        "dns_self_heal_empty" => "尚未观测到 DNS 健康事实".into(),
        "dns_self_heal_healthy" => "正常".into(),
        "dns_self_heal_warning" => "注意".into(),
        "dns_self_heal_critical" => "故障".into(),
        "dns_self_heal_unknown" => "未观测".into(),
        "dns_self_heal_listen_port" => "监听端口 (dns.listen)".into(),
        "dns_self_heal_upstream" => "上游解析可达性".into(),
        ...
        "custom_node_params_title" => "协议参数（共享草稿）".into(),
        "custom_node_alpn" => "ALPN 协商顺序".into(),
        "custom_node_ech_config" => "ECH 配置 (base64)".into(),
        "custom_node_tuic_cc" => "TUIC 拥塞控制".into(),
        "custom_node_tuic_udp_relay" => "UDP 中继模式".into(),
        "custom_node_tuic_reduce_rtt" => "减少 RTT".into(),
        "custom_node_tuic_heartbeat" => "心跳间隔 (ms)".into(),
        "custom_node_tuic_request_timeout" => "请求超时 (ms)".into(),
        "custom_node_recv_window" => "接收窗口".into(),
        "custom_node_hy2_ports" => "端口跳跃 (mport)".into(),
        "custom_node_hy2_hop" => "跳跃间隔 (s)".into(),
        "custom_node_hy2_cwnd" => "拥塞窗口".into(),
        "custom_node_hy2_udp_mtu" => "UDP MTU".into(),
        "custom_node_hy2_obfs" => "Hysteria2 混淆".into(),
        "custom_node_hy2_obfs_password" => "混淆密码".into(),
        "custom_node_hy2_up" => "上行带宽".into(),
        "custom_node_hy2_down" => "下行带宽".into(),
        "custom_node_wg_private_key" => "WireGuard 私钥".into(),
        "custom_node_wg_public_key" => "对端公钥".into(),
        "custom_node_wg_preshared_key" => "预共享密钥".into(),
        "custom_node_wg_reserved" => "Reserved".into(),
        "custom_node_wg_ip" => "内网地址".into(),
        "custom_node_wg_mtu" => "MTU".into(),
        "custom_node_wg_keepalive" => "保活 (s)".into(),
        ...
        "custom_node_transport_network" => "传输层".into(),
        "custom_node_ws_path" => "WS 路径".into(),
        "custom_node_ws_host" => "WS Host 头".into(),
        "custom_node_ws_early_data" => "WS 0-RTT 提前数据".into(),
        "custom_node_grpc_service" => "gRPC 服务名".into(),
        "custom_node_xhttp_mode" => "XHTTP 模式".into(),
        "custom_node_xhttp_path" => "XHTTP 路径".into(),
        "custom_node_plugin_name" => "SIP003 插件".into(),
        "custom_node_plugin_host" => "插件 host".into(),
        "custom_node_plugin_password" => "插件口令".into(),
        "custom_node_plugin_mode" => "插件 mode".into(),
        "custom_node_ssh_username" => "SSH 用户名".into(),
        "custom_node_ssh_private_key" => "SSH 私钥".into(),
        "custom_node_ssh_passphrase" => "SSH 私钥口令".into(),
        "custom_node_ssh_algorithms" => "Host Key 算法".into(),
        "custom_node_anytls_idle" => "AnyTLS 空闲超时 (ms)".into(),
        "custom_node_anytls_idle_check" => "AnyTLS 空闲检查 (ms)".into(),
        "custom_node_anytls_min_idle" => "AnyTLS 最小空闲会话".into(),
        ...
        "mixin_studio_toggles_title" => "常用覆写开关".into(),
        "mixin_toggle_ipv6" => "开启 IPv6".into(),
        "mixin_toggle_allow_lan" => "允许局域网".into(),
        "mixin_toggle_dns_fake_ip" => "注入 DNS fake-ip".into(),
        "mixin_toggle_tun" => "注入 TUN".into(),
        "mixin_toggle_sniffer" => "注入协议嗅探".into(),
        "mixin_toggle_log_debug" => "调试日志级别".into(),
        "mixin_studio_preflight_ok" => "Mixin 预检通过".into(),
        "mixin_studio_preflight_blocked" => "Mixin 预检阻断".into(),
        "mixin_studio_preflight_hint" => "语法、合并与最终配置校验均已通过，保存即生效".into(),
        "mixin_studio_cascade_title" => "覆写流水线".into(),
        "mixin_cascade_base" => "Base 配置".into(),
        "mixin_cascade_subscription" => "订阅配置".into(),
        "mixin_cascade_merge" => "Merge 规则".into(),
        "mixin_cascade_pre_mixin" => "Pre-Mixin".into(),
        "mixin_cascade_post_mixin" => "Post-Mixin".into(),
        "mixin_studio_cascade_undeclared" => "未声明".into(),
