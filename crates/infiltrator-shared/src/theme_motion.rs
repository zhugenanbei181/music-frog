...
mod tests {
    ...
    fn test_build_theme_spec() {
        let standard = build_theme_spec(
            "light",
            ThemeContrast::Standard,
            MotionPreference::FullMotion,
        );
        ...
        let high_contrast = build_theme_spec(
            "hc",
            ThemeContrast::HighContrast,
        ...
        let amoled = build_theme_spec(
            "amoled",
            ThemeContrast::AmoledDark,
            MotionPreference::NoMotion,
        );
    ...
    fn test_compute_duration_ms() {
        assert_eq!(
            compute_duration_ms(1000, MotionPreference::FullMotion),
            1000
        );
        assert_eq!(
            compute_duration_ms(1000, MotionPreference::ReducedMotion),
            500
        );
    ...
    fn test_is_shimmer_enabled() {
        assert!(is_shimmer_enabled(&SkeletonState::Loading {
            estimated_ms: 1000,
            shimmer: true
        }));
        assert!(!is_shimmer_enabled(&SkeletonState::Loading {
        ...
        assert!(!is_shimmer_enabled(&SkeletonState::Failed {
            retry_allowed: true
        }));
    }
    ...
    fn test_serialization() {
        ...
        let state = SkeletonState::Loading {
            estimated_ms: 500,
            shimmer: true,
        };
