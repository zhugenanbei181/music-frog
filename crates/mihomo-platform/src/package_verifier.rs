...
impl PackageManifestVerifier {
    ...
    pub fn parse_release_manifest(
        manifest_json: &str,
    ) -> Result<(String, Vec<PackageArtifactInfo>)> {
        let manifest: ReleaseManifest =
            serde_json::from_str(manifest_json).context("Failed to parse release manifest JSON")?;
...
mod tests {
    ...
    fn test_verify_sha256() {
        ...
        assert!(PackageManifestVerifier::verify_sha256(
            data,
    ...
    fn test_infer_package_type() {
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("app_installer.msi"),
            Some(PackageType::WindowsMsi)
        );
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("setup.exe"),
            Some(PackageType::WindowsNsisExe)
        );
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("MusicFrog.AppImage"),
            Some(PackageType::LinuxAppImage)
        );
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("package.deb"),
            Some(PackageType::LinuxDeb)
        );
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("installer.dmg"),
            Some(PackageType::MacOsDmg)
        );
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("app.apk"),
            Some(PackageType::AndroidApk)
        );
        assert_eq!(
            PackageManifestVerifier::infer_package_type_from_filename("source.tar.gz"),
            None
        );
    }
    ...
    fn test_manifest_round_trip() {
        ...
        // Parse back
        let (parsed_version, parsed_artifacts) =
            PackageManifestVerifier::parse_release_manifest(&manifest_json)
