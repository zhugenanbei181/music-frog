...
mod tests {
    ...
    fn test_asset_filename_formatting() {
        let win_filename =
            get_upstream_download_filename("x86_64-pc-windows-msvc", "v1.18.3").unwrap();
        assert_eq!(win_filename, "mihomo-windows-amd64-v1.18.3.zip");

        let mac_filename =
            get_upstream_download_filename("aarch64-apple-darwin", "v1.18.3").unwrap();
        assert_eq!(mac_filename, "mihomo-darwin-arm64-v1.18.3.gz");

        let linux_filename =
            get_upstream_download_filename("x86_64-unknown-linux-gnu", "v1.18.3").unwrap();
