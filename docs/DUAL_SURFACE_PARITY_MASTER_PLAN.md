...
| 组 09 AST YAML 引擎与快照 Diff | 15 | `in progress (14/15)` | `yaml_edit/`（domain）、`snapshot_diff_modal.rs`（Iced）、`profiles_diff.rs`/`profiles_editor.rs`（Bevy） | 2026-09-22 三批展开（见组 09 逐项账目）：01–13/15 已双端收口（14/15 `parity-ready`）；14 为 `shared-ready`（编辑器已双端同源：行号槽/有界窗口/片段栏齐备，但 Bevy 无 Mixin/Filter/Script 覆盖面板） |
...
| `DUAL-09-14` | 双端编辑器与 Diff 模态完全镜像 | `shared-ready` | 本批把编辑器半边再收窄一格：Iced 现在也有**行号槽 + 有界视口窗口**（`view/editor_viewport.rs`，行号按共享 `EditorViewport` 渲染、`Action::Scroll` 镜像 + 光标跟随），两端共有同一片段目录与同一格式化器/预检/守卫保存；Diff 半边保持同源（同一 `YamlAstDiffSnapshot`、内联/并排、两步回滚，Bevy 可按任意历史条目出 diff）。但**仍不宣称“完全镜像”**：Bevy 编辑器只编辑当前生效配置文档，没有 Mixin/Filter/Script 三个覆盖面板（Iced 有 4 个面板），片段栏虽已双端但 Bevy 没有 Filter 表单；两端 Diff 呈现形态仍是 Iced 模态 vs Bevy 卡片。故维持 `shared-ready` |
