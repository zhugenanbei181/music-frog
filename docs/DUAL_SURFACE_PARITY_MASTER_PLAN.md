...
| :--- | :---: | :--- | :--- | :--- |
| 组 05 协议生态保真与多路复用 | 15 | `in progress (12/15)` | `protocol_fidelity.rs`+`protocol_params.rs`（contract）、`protocol_node_projection.rs`+`protocol_codec_application.rs`（application）、`profile_converter`/`proxy_nodes`（domain）、`custom_node_modal.rs`+`custom_node_params.rs`（Iced）、`proxies_custom.rs`（Bevy） | 2026-09-22 两批展开（见组 05 逐项账目）：01/02/03/04/05/06/07/08/11/12/14/15 已双端收口（12/15）；09/10/13 `planned`（09 仅 typed `dialer-proxy` 字段无链路拓扑，10 无 dialer 依赖图环路检测，13 全仓无自定义 CA 载体） |
...
| `DUAL-05-09` | 前置跳板代理（Dialer-Proxy）链路 | `planned` | 仅存在 typed 字段 `dialer_proxy`（8 个 `ProxyNode` 变体各一处，`model.rs:382`/`:430`/`:487`/`:530`/`:561`/`:599`/`:631`/`:673`）与 flat `ProxyNodeItem.dialer_proxy`（`profile_converter.rs:145`）；全仓无链路解析（`dialer-proxy` 值 → 节点）、无拓扑投影、无双端可视化。URI 编解码不读写该字段。matrix 以 `covered:false` 登记真实原因 |
| `DUAL-05-10` | 环路依赖静态检测 | `planned` | 存在 `detect_group_cycles`（`crates/infiltrator-domain/src/rules/analyzer.rs:256`，Rule Tracer 的 proxy-group 依赖 DFS），但**没有任何调用方把它接到 `dialer-proxy` 链或 Relay 策略组**；无 dialer 依赖图构建、无双端警示阻断。属组 11 已收口能力，组 05 侧的 dialer 环路仍未实现。matrix 以 `covered:false` 登记真实原因 |
...
| `DUAL-05-13` | 自定义 CA 证书与证书白名单 | `planned` | 全仓无自定义 CA / 证书白名单字段（`grep -rn "ca_str\|certificate_authority\|custom_ca" crates/` 零命中），无导入端口、无指纹校验、双端无入口。matrix 以 `covered:false` 登记真实原因 |
