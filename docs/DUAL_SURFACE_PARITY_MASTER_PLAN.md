...
| 组 15 多模态外壳与极客命令流 | 15 | `in progress (8/15)` | `mini_hud.rs`、`command_palette.rs`、`sidebar.rs` | 2026-09-22 四批展开（见组 15 逐项账目）：01/03/04/05/06/08/09/12 已双端收口（8/15），14/15 为 `shared-ready`，01 由 [RESPONSIVE_PARITY_LEDGER.md](RESPONSIVE_PARITY_LEDGER.md) 权威跟踪、矩阵见 [MULTIMODAL_SHELL_MATRIX.md](MULTIMODAL_SHELL_MATRIX.md)，02/07/10/11/13 `planned` |
...
| `DUAL-15-14` | 双端设计规范与交互质感对齐 | `shared-ready` | 视觉口径收敛为**可机检的共享 token**：`infiltrator-contract/src/design_tokens.rs`（四皮肤核心调色板 `skin_core(RgbaToken)`：canvas/sidebar/card/card_border/control_bg/ink/ink_dim/accent/on_accent/success/warning/danger + 间距阶梯 4/8/12/16/20/24 + 圆角 16/10 + 发丝线 1px）；Iced `view/theme.rs` 的 `LIGHT/DARK/FOREST/AMOLED` 与 `SP_*`/`R_*`/`HAIRLINE` 全部由契约常量构造（编译期绑定），整个 Iced 前端的 1px 边框已从 `width: 1.0` 字面量扫成 `theme::HAIRLINE`（本批 25 个文件、69 处），`the_iced_tokens_resolve_the_shared_design_contract` 逐通道断言、`theme_hairline_consumes_the_shared_contract_metric` 断言数值；业务无关的 Bevy widget 层按数值镜像（dark/light 旧 iOS 色板与 forest/amoled 偏亮填充、圆角 12/8 已在批次 C 对齐为 16/10），由 `the_widget_palette_mirrors_the_shared_design_tokens`/`the_widget_ladders_mirror_the_shared_contract_numbers` 与守卫 `multimodal-shell-guard.py::check_design_token_mirrors`（逐 token 解析两侧源码数值）+ `check_no_raw_hairlines`（禁止 Iced 回退裸 1.0）三重锁定。**缩小的声明范围**：overlay 气泡、hover/pressed 洗色、icon tile 染色、阴影与字号阶梯仍是各表面本地装饰；02/07/10/11/13 未双端闭环，1:1 质感对齐未达成，保持 `shared-ready` |
| `DUAL-15-15` | 多模态与外壳架构无头测试矩阵 | `shared-ready` | 可机检矩阵 [MULTIMODAL_SHELL_MATRIX.md](MULTIMODAL_SHELL_MATRIX.md)：15 项逐行绑定 `路径::测试名` evidence，守卫逐行验证（a）引用文件存在且测试名真实出现，（b）`parity-ready` 行必须有共享/应用 + Iced + Bevy **三侧** evidence 且不少于三个标记，（c）`planned` 行不得声称测试，（d）每行状态必须等于主计划组 15 逐项账目的状态（两文档不允许不一致）。已闭环项（01/03/04/05/06/08/09/12）三侧无头断言齐备，14/15 为 `shared-ready` 并注明偏差来源；02/07/10/11/13 无测试面，矩阵未 100% |
