...
| 组 11 规则引擎与 MRS 治理 | 15 | `parity-ready (15/15)` | `rules.rs`、`rules_mrs.rs`、`mrs` | 2026-09-22 五批收口（见组 11 逐项账目）：01/02/03/04/06/07/08/09/10/11/12/13/14/15 双端收口；2026-09-23 收口 05（内核真实 `etag-support` 顶层开关 + 本地内容指纹 + `updatedAt` + 声明 interval 的合成叙事，15/15）。**诚实边界**：逐请求 304 结果仍只在 mihomo 内核内、控制器不返回校验器 |
...
| 组 14 DNS 工作台与泄漏探活 | 15 | `parity-ready (15/15)` | `dns.rs`（两端） | 2026-09-23 起逐项展开（见组 14 逐项账目）：01/02/03/04/05/06/07/08/09/10/11/12/13/14/15 已双端收口（15/15 `parity-ready`），08 由真实公共 TXT 回显权威区收口（默认 `whoami.ds.akahelp.net` 的 `"ip"` 键值对 + `o-o.myaddr.l.google.com` 首个 IP，经 `system` 解析路径，应用仍比较、不硬编码结论）、09 经**重新界定**为真实 STUN UDP 出网映射探测（`stun.l.google.com:19302`，本机/本进程视角，非浏览器 WebRTC 结论） |
...
| `DUAL-11-05` | 外部规则集增量更新与 ETag 缓存 | `parity-ready` | 共享 `CommandIntent::RefreshRuleProviders`（`infiltrator-contract/src/command.rs:234`）经 `command_application.rs:484` 逐 provider 调 `RuntimeGateway::update_rule_provider`（真实 `PUT /providers/rules/{name}`）；Iced `Message::UpdateRuleProvider`（`update/core/rules.rs:704`）与 Bevy `RefreshRuleProvidersButton`（`pages/rules.rs:357`）同源。共享 `RuleProviderSnapshot.refresh_interval_secs`（reader 读活动 profile `rule-providers.<name>.interval`，未声明为 `None`，`surface_reader.rs`）、共享 `view::format_refresh_interval`，Iced `provider_lifecycle_line` 渲染 `Auto: 1d (kernel-scheduled)` / `Auto: not declared`（`state.rs rule_provider_intervals`），Bevy `provider_updated_label` 渲染「自动刷新: 1d (内核调度)/未声明」；双端测试 Iced `test_provider_lifecycle_line_reports_shared_source_url`、Bevy `test_rules_provider_lifecycle_renders_shared_source_url`、领域 `matrix_11_05_provider_refresh_intent_and_declared_interval`。**2026-09-23（DUAL-11-05 本地事实）**：新增客户端真实可测的一半——共享 `ProviderFileFingerprint`（size + SHA-256 + last-modified）与枚举 `ProviderFingerprintChange` 的 `ProviderCacheFingerprint::compare`（`first-seen`/`unchanged`/`changed`，内容判定，mtime 单独前进不算内容变化）落在 `infiltrator-contract/src/provider_cache.rs`；`RuleProviderCachePort::fingerprint`（`infiltrator-desktop/src/rule_provider_cache.rs`）读取内核真实缓存文件 `rules/<md5(url)>`，`sha2` digest 仅在 size/mtime 与上次不同时重算（未变输入捷径，无 mtime 时每次真读），`RuleProviderApplication::observe_fingerprint` 与上一次本地观测比较后经 `RuleProviderSnapshot.cache_fingerprint` 下发；Iced `provider_fingerprint_line` 渲染「本地缓存内容指纹（非 HTTP ETag）: sha256:… · N B · mtime … · 首次观测/较上次观测未变化/较上次观测已变化」，Bevy `provider_updated_label` 同源渲染；双端/领域/应用/宿主测试 Iced `test_provider_fingerprint_line_reports_local_file_facts`、Bevy `test_rules_provider_local_cache_fingerprint_renders_non_etag_label`、domain `content_fingerprint_formats_digest_size_and_mtime`、application `fingerprint_observation_compares_against_the_previous_local_read`、desktop `fingerprint_reports_real_size_digest_and_mtime_for_the_cache_file`。locale key `rules_provider_fingerprint_label` 双表齐备。**诚实边界**：该指纹是**本地缓存文件内容指纹**，不是 HTTP `ETag`/`If-None-Match`，也不声称内核跳过下载；内核 304 事务仍只在 mihomo 内可见（`PUT` 不返回校验器、`/providers/rules` 不暴露），故本项维持 `shared-ready`。**2026-09-23（内核真实 ETag 能力）**：先实证内核语义——`vendor/mihomo.exe` 含 `ETagSupport'yaml:"etag-support" json:"etag-support"` 与 `github.com/metacubex/mihomo/component/profile/cachefile/etag.go` 的 `GetETagWithHash`/`SetETagWithHash`；上游 `config/config.go` 中 `RawConfig.ETagSupport bool` 映射 `General.ETagSupport`，`DefaultRawConfig()` 置 `ETagSupport: true`，且它是**顶层**键（不在 `profile:` 段；`StoreSelected`/`StoreFakeIP` 才是 `RawProfile`）。据此新增共享事实 `KernelEtagSupportState`（`Enabled`/`Disabled`/`NotDeclared`，`as_str` 稳定 token）与 `KernelEtagSupportSnapshot { state, declared: Option<bool> }`（`infiltrator-contract/src/provider_cache.rs`，`from_declared` 只按真实声明分类、不臆造内核默认），经 `RulesPageSnapshot.etag_support` 下发；`ConfigurationApplication::load_etag_support`（`configuration_application.rs`）从活动 profile 顶层读 `etag-support`，`surface_reader.rs` 发布。Iced `etag_support_line`（`view/rules.rs`）在 Providers 视图渲染「ETag 缓存: 内核已启用 / 内核未启用 / 未声明」，Bevy `etag_support_label`（`pages/rules.rs`）在规则集卡片同源渲染；locale key `rules_etag_support_label`/`_enabled`/`_disabled`/`_not_declared` 中英双表齐备。**合成客户端叙事**：全局 `etag-support` 开关 + 内核 `updatedAt` + 声明 `interval`（`kernel-scheduled`）+ 本地内容指纹，四者同屏。证据：contract `etag_support_classifies_the_raw_declaration_without_inventing_one`、application `load_etag_support_reads_the_top_level_declaration_honestly`、domain `matrix_11_05_provider_refresh_intent_and_declared_interval`、Iced `test_etag_support_line_reports_the_declared_kernel_capability` 与 `test_rules_provider_interval_and_publish_truncation_project_from_snapshot`、Bevy `test_rules_provider_etag_support_renders_declared_kernel_capability`。**诚实边界**：客户端发布的是**声明**（顶层 `etag-support` 与本地文件指纹），逐请求 `304` 结果仍在 mihomo 内核内（`PUT /providers/rules/{name}` 不返回校验器、`/providers/rules` 不暴露），故 `NotDeclared` 绝不冒充显式开关，任一面都不渲染 304 命中/未修改；本项据此升为 `parity-ready` |
...
| `DUAL-14-09` | WebRTC 公网 IP 穿透探测（**重新界定**为本机 UDP 出网映射） | `parity-ready` | **重新界定（2026-09-23，用户批准）**：本项不再声称观测浏览器 WebRTC 栈；改为由宿主真实发出 STUN Binding Request，观测**本机/本进程的 UDP 出网映射**（外部 STUN 服务器看到的公网 IP:端口），并与期望的代理出网做事实比较。**共享契约** `infiltrator-contract/src/stun_probe.rs`：`StunProbeStatus{Unknown,Observed,TimedOut,Failed{message},Unsupported{reason}}`、`StunProbeTransport{Udp,Unsupported{reason}}`、`StunMappedAddress{ip,port}`、`StunProbeObservation{server,transport,status,mapping}`、`StunProbeReport{server,transport,status,mapping,expected_egress}`、`StunEgressComparison{Consistent,Divergent,Unknown{reason}}`；`comparison()` 仅比较观测与期望的 **IP**（端口是探测套接字的临时源端口，无稳定含义），任一未知一律 `Unknown`（`NO_OBSERVED_MAPPING_REASON`/`NO_EXPECTED_EGRESS_REASON`），不一致只报 `Divergent` 事实、绝不判定泄漏；写入 `DnsPageSnapshot.stun`（`surface_snapshot.rs`）。**宿主端口** `ports/src/stun_probe.rs`：`StunProbePort::observe(StunProbeRequest)->StunProbeObservation`（宿主能力，真实发出 UDP 绑定请求）与 `StunEgressProbePort::probe()->StunProbeReport`（双端驱动的共享应用边界），`HostRuntime::stun_egress_probe_port` 默认 `None`（默认 typed unsupported）。**真实适配器** `core/src/stun_wire.rs` + `core/src/stun_io.rs::UdpStunProbe`：手工编码 RFC 5389 Binding Request（20 字节头 + magic cookie `0x2112A442` + 96 位事务 id，splitmix64 无新依赖），`connect` 固定 STUN 服务器后 `send`/`recv`，严格校验 magic cookie/声明长度/事务 id 后解析 **XOR-MAPPED-ADDRESS**（IPv4/IPv6 均按 RFC 5389 §15.2 异或还原；刻意不读已废弃的 MAPPED-ADDRESS），非本事务数据报丢弃重等、服务器 ERROR-CODE 如实 `Failed`；超时 `TimedOut`、DNS/套接字错误 `Failed`，绝不伪造地址。**共享应用** `application/src/stun_probe_application.rs::StunProbeApplication`：持有可配置服务器（默认 `stun.l.google.com:19302`）、可配置**期望代理出网 IP**（`with_expected_egress` 仅接受可解析 IP，否则保持 `None`）与共享 `last_report()`；`impl StunEgressProbePort` 使任一端发起的探测都落进同一份报告；无端口 typed `Unsupported`。**读取/命令**：`dns_workbench_application::stun_report` + `surface_reader::with_stun_probe` 填充快照；`CommandIntent::RunStunProbe`（`command_application::run_stun_probe`）驱动同一应用；桌面 `runtime.rs`/`boot.rs` 注入 `UdpStunProbe` + `DEFAULT_STUN_SERVER`，`surface.rs`/`composition.rs` 共享同一实例。**Iced**：`update/core/stun.rs`（`Message::RunStunProbe` → `Message::StunProbed`，只经 `stun_egress_probe_port` 触发）+ `view/stun_probe_panel.rs::stun_panel`/`stun_status_copy`/`stun_comparison_copy`（zh/en 全量 i18n `dns_stun_*`，含把「这是本机/本进程的 UDP 出网映射，不是浏览器 WebRTC 穿透结论」直接渲染在面板上）。**Bevy**：`pages/dns_stun.rs`（`stun_conclusion_label`/`stun_mapping_listing`/`dns_stun_card_scene`/`DnsLineKind::Stun`）+ 头卡 `TestStunProbeButton` → `UiCommand::RunStunProbe` → `CommandIntent::RunStunProbe`；demo fixture 显式标注为截图夹具。**测试**：契约 9 项（`a_default_report_is_unknown_and_never_a_verdict`/`an_observed_mapping_without_an_expected_egress_is_unknown`/`matching_ips_are_consistent_and_the_port_is_ignored`/`differing_ips_are_divergent_facts_not_a_leak_verdict`/`a_failed_or_timed_out_probe_never_compares`/`an_unsupported_host_is_a_typed_refusal_not_a_result`/`requests_keep_a_usable_deadline_and_the_server` 等）、应用 8 项（`a_host_without_a_stun_prober_reports_typed_unsupported_and_probes_nothing`/`an_observed_mapping_and_expected_egress_are_consistent`/`a_differing_mapping_is_a_divergent_fact_not_a_leak_verdict`/`a_host_without_an_expected_egress_publishes_an_unknown_comparison`/`a_timed_out_probe_keeps_the_typed_status_and_no_mapping`/`a_prober_error_never_overwrites_the_last_honest_report`/`an_unusable_expected_egress_stays_unknown_instead_of_fabricating_one` 等）、core `stun_wire` 11 项 + 适配器 6 项（**真实回环 UDP 应答器**返回手工构造的 Binding Success，含 IPv4/IPv6、错事务号数据报被忽略、静默超时、ERROR-CODE typed 失败、不可用地址不伪造映射，以及 `#[ignore]` 真实公共 STUN 服务器验证）、Iced `test_stun_panel_renders_the_shared_report_honestly`/`test_stun_panel_names_the_non_webrtc_boundary`、Bevy `test_dns_stun_submits_command`/`test_dns_stun_card_renders_the_shared_egress_report`；守卫 `dns-studio-guard.py` 已扩展（require 全链路 marker + forbid 把本机 STUN 结果表述成 `WebRTC 泄漏`/`WebRTC leak`）。**诚实边界（重新界定的精确含义）**：①测得的是**本机/本进程**的 UDP 出网映射，外部 STUN 服务器只看到这条 UDP 流的源地址；**不是**任何浏览器 ICE/WebRTC 栈的穿透结论，本产品不内置浏览器栈、也不声称观测到它；②比较对象是「期望的代理出网 IP」，默认桌面**未注入**该期望值，因此默认 `comparison()` 为 `Unknown`（面板如实显示「无法比较: no expected proxied egress is known on this host」），不把缺失的期望当作「无泄漏」；③即便期望存在且不一致，也只报 `Divergent` 事实（观测 vs 期望），不判定哪一个是泄漏、不下安全结论；④只编码/解析 XOR-MAPPED-ADDRESS，刻意忽略已废弃的 MAPPED-ADDRESS（RFC 5389 建议），不鉴权、不覆盖 STUN 的完整属性集（对公开匿名 Binding 足够）；⑤事务 id 由时间/计数器/pid 的 splitmix64 派生，仅用于匹配应答，非安全令牌；⑥`#[ignore]` 的公网测试是唯一真实网络路径，离线套件用本机 UDP 回环应答器覆盖协议正确性 |
...
> **2026-09-23 组 14 批次 G**：`DUAL-14-09` 由 `planned` 收口为 `parity-ready`——
> 但**重新界定**而非原样交付（组 14 现 15/15）。用户批准把本项从「观测浏览器 WebRTC
> 栈」改成「宿主真实发出 STUN Binding Request，观测**本机/本进程的 UDP 出网映射**」：
> `infiltrator-contract::stun_probe` 给出 `StunProbeStatus{Observed,TimedOut,Failed,
> Unsupported,Unknown}`、`StunMappedAddress{ip,port}`、`StunProbeObservation`、
> `StunProbeReport` 与 `StunEgressComparison{Consistent,Divergent,Unknown}`（`comparison()`
> 只比较 IP，任一未知为 `Unknown`，不一致只报事实）；`infiltrator-core::stun_wire` 手工
> 编码/解析 RFC 5389 Binding Request 与 XOR-MAPPED-ADDRESS（无新依赖），
> `infiltrator-core::stun_io::UdpStunProbe` 真实走 UDP；`StunProbeApplication` 以默认
> `stun.l.google.com:19302` 驱动并经 `DnsPageSnapshot.stun` 到达 Iced 与 Bevy 面板
> （`dns_stun_*` / `dns_stun_card_scene`），双端都把「这是本机/本进程的 UDP 出网映射，
> 不是浏览器 WebRTC 穿透结论」渲染在界面上。离线套件用本机 UDP 回环应答器覆盖
> Binding Success（IPv4/IPv6）、错事务号忽略、静默超时与 typed 失败；真实公网 STUN
> 仅 `#[ignore]` 验证。守卫 `dns-studio-guard.py` 增补全链路 marker 并 forbid 把本机
> 结果表述成 `WebRTC 泄漏`/`WebRTC leak`。**诚实边界**：应用侧 STUN 永远看不到浏览器
> 的穿透行为；默认桌面未注入「期望代理出网」，比较如实 `Unknown`；`Divergent` 只列
> 观测与期望的事实，不判定泄漏。
