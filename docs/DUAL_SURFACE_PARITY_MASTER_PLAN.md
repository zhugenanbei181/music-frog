...
| :--- | :---: | :--- | :--- | :--- |
| 组 05 协议生态保真与多路复用 | 15 | `in progress (4/15)` | `protocol_fidelity.rs`（contract）、`protocol_codec_application.rs`（application）、`profile_converter`（domain）、`custom_node_modal.rs`（Iced）、`proxies_custom.rs`（Bevy） | 2026-09-22 起逐项展开（见组 05 逐项账目）：01/02/11/14 已双端收口；03/04/05/06/07/08/12/15 为 `shared-ready`（domain 解析/校验/序列化齐备但无共享草稿或双端编辑面）；09/10/13 `planned`（09 仅 typed `dialer-proxy` 字段无链路拓扑，10 无 dialer 依赖图环路检测，13 全仓无自定义 CA/ECH 载体） |
...
| 组 09 AST YAML 引擎与快照 Diff | 15 | `in progress (11/15)` | `yaml_edit/`（domain）、`snapshot_diff_modal.rs`（Iced）、`profiles_diff.rs`/`profiles_editor.rs`（Bevy） | 2026-09-22 两批展开（见组 09 逐项账目）：01/03/05/06/07/08/09/10/11/12/15 已双端收口（11/15 `parity-ready`）；14 为 `shared-ready`（编辑器已双端同源，但 Bevy 无 Mixin/Filter/Script 面板、Iced 无行号槽）；02/04/13 `planned`（缩进参考线/片段目录/虚拟滚动） |
...
| 组 15 多模态外壳与极客命令流 | 15 | `in progress` | `mini_hud.rs`、`command_palette.rs`、`sidebar.rs` | 2026-09-22 起逐项展开（见组 15 逐项账目）：04/05/06/08/09/12 已双端收口，03/14/15 为 `shared-ready`，01 由 [RESPONSIVE_PARITY_LEDGER.md](RESPONSIVE_PARITY_LEDGER.md) 权威跟踪、矩阵见 [MULTIMODAL_SHELL_MATRIX.md](MULTIMODAL_SHELL_MATRIX.md)，其余 `planned` |
...
| `DUAL-05-03` | TUIC v5 与 Hysteria 2 拥塞控制/端口跳跃/masquerade | `shared-ready` | domain typed `TuicNode.congestion_controller`/`udp_relay_mode`/`reduce_rtt`（`model.rs:439`）、`Hysteria2Node.ports`/`obfs`/`up`/`down`/`cwnd`（`model.rs:390`）与白名单校验（`validate.rs:14`、`:15`）；`PortHopping` 端口跳跃解析（`model.rs:187`）；URI 解析 `tuic://`（`uri_parse.rs:351`）、`hysteria2://` 含 `mport`（`uri_parse.rs:260`）与导出（`uri_export.rs:212`）。缺口：共享草稿/报告未承载拥塞与伪装字段，双端无编辑面；**全仓无 `masquerade` 字段**（`grep -rn masquerade crates/` 仅命中无关注释），故不计入 `parity-ready` |
| `DUAL-05-04` | WireGuard / AmneziaWG 全参数 | `shared-ready` | domain typed `WireGuardNode` + `AmneziaOpts`（jc/jmin/jmax/s1/s2/h1-h4，`model.rs:115`、`:496`）、`Reserved` 双形态保形（`model.rs:337`，list 与 base64 原样回写）与 base64 32 字节校验（`validate.rs:164`）；URI 解析/导出 `wireguard://`/`awg://`（`uri_parse.rs:484`、`uri_export.rs:335`）。缺口：共享草稿未建模 WireGuard 密钥族，双端无参数编辑面；peers 列表仅以松散 `Value` 承载 |
| `DUAL-05-05` | 现代传输层（XHTTP/gRPC/QUIC/WS 0-RTT） | `shared-ready` | domain typed `XhttpOpts`（mode/path/host/headers，`model.rs:165`）与 xhttp mode 白名单校验（`validate.rs:19`）；flat 节点承载 `ws_opts`/`grpc_opts`/`h2_opts`/`http_opts`/`xhttp_opts`（`crates/infiltrator-domain/src/profile_converter.rs:155-163`）与 `packet_encoding`；WS Early Data（`max-early-data`）仅存在于 mihomo 读模型 `WsOpts`（`crates/infiltrator-domain/src/proxy.rs:94`），**不在配置编辑链路上**；原生 QUIC 传输无独立建模。缺口：共享草稿无传输层子结构、双端无传输编辑面 |
| `DUAL-05-06` | SIP003 插件链生态 | `shared-ready` | flat 节点 `plugin`/`plugin_opts` 解析（SIP003 `;` 参数展开，`crates/infiltrator-domain/src/profile_converter/uri_parse.rs:203` 区块）与导出重组（`uri_export.rs:36`）；typed `ShadowsocksNode.plugin`/`plugin_opts`（`model.rs:549`）；domain 测试 `test_parse_shadowsocks_sip002_and_sip003`。缺口：无插件链协同校验（如 shadow-tls 与 ss 的版本互斥）、共享草稿无插件块、双端无插件编辑面 |
| `DUAL-05-07` | 原生 SSH SOCKS 代理 | `shared-ready` | URI 解析 `ssh://` 含 user/private-key/passphrase/host-key-algorithms（`uri_parse.rs` ssh 分支）与导出（`uri_export.rs:448`）；flat 节点 typed 字段（`profile_converter.rs:139-143`）。缺口：typed `ProxyNode` 枚举**无 SSH 变体**（`model.rs:696`），SSH 节点降级为 `OtherNode` 无损但无类型化字段；`validate_item` 无 ssh 凭据规则；双端无 SSH 编辑面 |
| `DUAL-05-08` | AnyTLS 与 Trojan-Go 接入 | `shared-ready` | domain typed `AnytlsNode`（password/idle_timeout/padding_range/reality，`model.rs:569`）与 padding 区间校验（`validate.rs:273`）；URI 解析/导出 anytls（`uri_export.rs:403`）与 domain 测试 `test_anytls_uri_roundtrip`；`trojan-go` 仅在 family 映射中被识别（`protocol_fidelity.rs` `ProtocolFamily::from_type_str`），typed 模型无 `trojan-go` 变体。缺口：双端无 anytls/trojan-go 编辑面 |
| `DUAL-05-09` | 前置跳板代理（Dialer-Proxy）链路 | `planned` | 仅存在 typed 字段 `dialer_proxy`（8 个 `ProxyNode` 变体各一处，`model.rs:382`/`:430`/`:487`/`:530`/`:561`/`:599`/`:631`/`:673`）与 flat `ProxyNodeItem.dialer_proxy`（`profile_converter.rs:145`）；全仓无链路解析（`dialer-proxy` 值 → 节点）、无拓扑投影、无双端可视化。URI 编解码不读写该字段 |
| `DUAL-05-10` | 环路依赖静态检测 | `planned` | 存在 `detect_group_cycles`（`crates/infiltrator-domain/src/rules/analyzer.rs:256`，Rule Tracer 的 proxy-group 依赖 DFS），但**没有任何调用方把它接到 `dialer-proxy` 链或 Relay 策略组**；无 dialer 依赖图构建、无双端警示阻断。属组 11 已收口能力，组 05 侧的 dialer 环路仍未实现 |
| `DUAL-05-11` | 多路复用（Smux/Yamux/H2Mux）调优 | `parity-ready` | 共享 `SmuxParams`（enabled/protocol/max-connections/min/max-streams/padding/statistic/only-tcp + `validate`/`chips`/`summary_zh`，`protocol_fidelity.rs:360`）；`ProtocolFidelityReport.smux_chips`/`smux_overrides`；`draft.smux → YAML smux:` 写入与回读（`protocol_codec_application.rs:133` 的 `node_from_draft`、`:120` 调用的 `smux_from_json`）；domain typed `SmuxOpts`（`model.rs:141`）。Iced 复用开关/协议/连接数/流数/填充输入（`custom_node_modal.rs:145`）；Bevy 芯片与 URI 损失行（`proxies_custom.rs:302`）。证据：contract `smux_parameters_are_typed_and_validated`、application `smux_overrides_are_yaml_only_and_reported_as_a_uri_gap`、Iced `draft_edits_re_derive_the_report_and_the_uri_gaps`、Bevy `custom_node_slots_render_the_shared_studio_facts`。**诚实标注**：分享链接格式（vless/ss/trojan/hy2/tuic URI）不携带 smux，该损失由 `uri_fidelity_gaps` 实测后双端展示，不宣称 URI 无损 |
| `DUAL-05-12` | TLS/ECH 与多版本 ALPN 协商 | `shared-ready` | ALPN 在 flat 节点（`profile_converter.rs:51`）、typed `VlessNode.alpn`（`model.rs:365`）、`ProtocolDraft.alpn` + 空项校验（`protocol_fidelity.rs:583`）与 URI 导出（trojan/tuic/hy2）齐备；`skip_cert_verify` 双端可见。缺口：**全仓无 ECH 载体**（无 `ech`/`ech-config` 字段或序列化），多版本 ALPN 仅按数组透传、无协商优先级模型；故不计入 `parity-ready` |
| `DUAL-05-13` | 自定义 CA 证书与证书白名单 | `planned` | 全仓无自定义 CA / 证书白名单字段（`grep -rn "ca_str\|certificate_authority\|custom_ca" crates/` 零命中），无导入端口、无指纹校验、双端无入口 |
| `DUAL-05-14` | 双向无损格式转换（URI/JSON/Clash YAML） | `parity-ready` | 共享应用 `ProtocolCodecApplication`（`crates/infiltrator-application/src/protocol_codec_application.rs:71` `draft_from_uri`、`:88` `uri_from_draft`、`:258` `upsert_draft_into_profile`、`:356` `audit_conversion`、`:192` `uri_fidelity_gaps`、`:434` `publish_studio`）与共享 `CodecAudit`/`ProtocolStudioSnapshot`（`protocol_fidelity.rs:884`、`:897`）；`upsert_draft_into_profile` 在 YAML value 层只替换同名节点、保留既有节点未知键并回填 `unknown_fields`，其它小节原样保留；**关键修复**：Iced 旧路径 `parse_nodes → export_nodes` 会丢掉 `proxy-groups`/`rules`/`dns` 等所有非 `proxies:` 小节，现已改为整文档 `upsert`（`crates/infiltrator-iced/src/update/protocol_codec.rs:99` 的 `SaveCustomNodeForm` → `:133` 的 `upsert_draft_into_profile`；`ui.rs:599` 仅做路由）；旧伪造导出（`ExportNodeAsUri` 用 `node.example.com`/`secret` 造节点）已删除并换成基于真实草稿的 `ExportCustomNodeUri`（`protocol_codec.rs:76`）；投影经 `pages.proxies.custom_node` 发布（`crates/infiltrator-contract/src/surface_snapshot.rs:190`、`crates/infiltrator-application/src/surface_reader.rs:395`）。证据：application `upsert_preserves_every_other_section_and_unknown_node_keys`/`yaml_round_trip_audit_is_honest_about_section_loss`/`upsert_refuses_a_draft_with_unresolved_protocol_issues`、Iced `the_upsert_path_the_modal_calls_preserves_the_whole_profile`/`export_never_fabricates_a_placeholder_node`、Bevy `custom_node_audit_line_reports_the_measured_lossless_verdict` |
| `DUAL-05-15` | 协议解析与序列化全量单测 | `shared-ready` | 解析/序列化无头覆盖已存在且本轮显著加厚：domain `crates/infiltrator-domain/src/profile_converter_test.rs`（9 协议 URI + 往返）、`crates/infiltrator-domain/src/proxy_nodes/proxy_nodes_test.rs`（typed 保真）、contract `protocol_fidelity_test.rs`（8 例）、application `protocol_codec_application_test.rs`（10 例）、Iced `tests/gui/protocol_codec_tests.rs`（9 例）、Bevy `tests/headless/protocol_codec_tests.rs`（4 例）。缺口：尚无组 06/12 那种**单一共享回归矩阵契约**（如 `ProtocolCodecMatrixReport`）逐协议逐字段断言全 15 项，故保持 `shared-ready` |
...
| `DUAL-09-02` | Monaco 级代码编辑器视口 | `planned` | Iced 有 `text_editor`（`crates/infiltrator-iced/src/view/editor.rs:256`）+ 语法错误行号胶囊（`format_syntax_line_pill`，`:80`）与等宽字体，仍无行号槽/缩进参考线/视口定位；本轮 Bevy 新增 `profiles_editor.rs` 的**行号槽 + 等宽 token 高亮 + 光标行窗口**（`crates/infiltrator-bevy-ui/src/pages/profiles_editor.rs`，`PROFILE_EDITOR_RENDER_LIMIT=240`），但无缩进参考线与虚拟滚动，Iced 侧仍无行号槽，本项维持 `planned`（双端不齐） |
...
| `DUAL-09-04` | 常用代码片段一键插入 (Snippets) | `planned` | 片段仍只在 Iced `view/editor.rs snip_btn` 与 `Message::InsertYamlSnippet`（`crates/infiltrator-iced/src/update/ui.rs:341`）；共享层无片段目录，Bevy 编辑器只有格式化/保存，无片段栏，本项不计入 |
...
| `DUAL-09-13` | 大文件编辑器性能优化 | `planned` | 仍无虚拟滚动/几何裁剪：Iced 无行号槽与视口 API，历史列表 `take(12)`；Bevy 编辑器虽然跟随光标只渲染 240 行窗口（`PROFILE_EDITOR_RENDER_LIMIT`）并在 UI 中直书“虚拟滚动属 DUAL-09-13，尚未实现”，但没有 10,000 行压测数据与 60 FPS 证据，不计入 |
| `DUAL-09-14` | 双端编辑器与 Diff 模态完全镜像 | `shared-ready` | 本轮把编辑器半边从“Iced 独占”推进为**双端同源**：两端都渲染共享 `ProfileDocumentSnapshot`、都用 `preflight_yaml_syntax` 实时预检、都用 `yaml_edit::format::format_yaml` 格式化、都经 `save_edited_profile_content` 的两步门禁保存（受保护订阅需显式解锁）、都显示同一 apply 事务结果；Diff 半边保持同源（同一 `YamlAstDiffSnapshot`、内联/并排、两步回滚），且 Bevy 现在可按任意历史条目出 diff（与 Iced 的逐条选择对齐）。但**不宣称“完全镜像”**：Bevy 编辑器只编辑当前生效配置文档，没有 Mixin/Filter/Script 三个覆盖面板与片段栏；Iced 没有行号槽/光标窗口；两端 Diff 呈现形态仍是 Iced 模态 vs Bevy 卡片。故维持 `shared-ready` |
...
| `DUAL-11-14` | 双端规则管理视口与组件 1:1 对等 | `planned` | 本轮后 Bevy 已具备 MRS 共享卡、视口搜索/分页、启停/调序、新增向导、游戏预设、可视逻辑子规则构建器、共享命中审计行，以及本轮补齐的真实 provider 解构入口与缓存清理事实行；仍缺 Iced 的 tabs 分区（列表/Providers/JSON 编辑器/Tracer 三页签 vs Bevy 单页长滚动）、GeoData 更新入口（`Message::UpdateGeoDatabases` 无 Bevy 对应控件）、JSON 编辑器分区；组件形态未 1:1，故维持 `planned`（11-06/07 已不再阻塞本项） |
...
| `DUAL-14-08` | DNS 泄漏多源并发交叉探测 | `planned` | **宿主事实缺失（未变）**：Iced `Message::RunDnsLeakProbe` 只从 `probe_public_ip` 取真实出口 IP，`country="US"`、`isp="Cloudflare"`、`is_leak_detected=false` 与 `tested_dns_servers` 均为硬编码，且无共享契约、Bevy 无对应面板；多源并发伪子域探测需要真实网络与多个第三方权威解析器，本轮无可用宿主事实源（`infiltrator-ports` 无 DNS 探测端口，`dns_tester` 只评估已解析结果），保持 `planned`，不伪造探测结论。精确阻塞点：需要新增「多源 DNS 泄漏探测」宿主端口 + 共享快照 + 双端面板，且探测必须由宿主真实发起 |
| `DUAL-14-09` | WebRTC 公网 IP 穿透探测 | `planned` | **宿主事实缺失（未变）**：领域 `PrivacyLeakDetectionSuite` 可基于连接/DNS 日志判定 STUN/TURN DIRECT 泄漏（`domain/diagnostics.rs:711`），`infiltrator-core::diagnostics_adapter` 已接；但无共享快照、无浏览器/STUN 实时探测能力（无 WebRTC/STUN 宿主端口），双端无该面板，保持 `planned`。精确阻塞点：STUN 绑定请求必须由宿主真实发出，当前端口层无该能力 |
| `DUAL-14-10` | DNS 解析测速与延迟高亮 | `shared-ready` | 共享 `DnsLatencyStatus`（`contract/src/dns.rs`，`Ready`/`Unsupported`）写入 `DnsPageSnapshot.latency`；两端渲染同一诚实状态：Iced `latency_policy_line`（`view/dns_hosts_panel.rs`，i18n `dns_latency_unsupported`/`dns_latency_ready`）、Bevy `latency_policy_label` + `DnsLineKind::LatencyPolicy`（`pages/dns_fakeip.rs`、`pages/dns.rs` servers 卡首行）；`CommandIntent::TestDnsLatency` 仍显式 `unsupported`（`command_application.rs:791`）；双端测试 `test_dns_latency_policy_line_is_honest`、`test_dns_latency_policy_line_is_localized_and_honest`。**诚实偏差（未升级为 parity-ready）**：无任何宿主提供逐 Nameserver 延迟事实——mihomo 控制器只有 `/dns/query`（整条解析链的往返，非单个 Nameserver），`dns_tester` 仅评估已解析结果且无执行器，`DnsTestResult.latency_ms` 恒为 `None`；因此本轮只发布 shared 的「不支持」事实与双端一致提示，**不填充任何假延迟**，状态为 `shared-ready` 而非 `parity-ready` |
...
| `DUAL-14-13` | DNS 故障自愈检测 | `planned` | **宿主事实缺失（未变）**：领域静态审计 `validate_dns_topology`（`domain/dns_topology.rs:24`）存在，宿主端口 `PortConflictPort` 只观测 `mixed-port`/`external-controller`（`contract/src/port_conflict.rs:PortBinding`），未覆盖 `dns.listen`；「上游无法解析」需要真实解析探测，端口层无该能力。本轮未新增共享 DNS 自愈快照，保持 `planned`。精确阻塞点：需要把 `dns.listen` 纳入端口占用观测并新增真实上游可达性探测端口，才能发布非伪造的自愈快照 |
...
| `DUAL-15-03` | 独立桌面 Mini HUD 悬浮小窗 | `shared-ready` | 共享只读模型 `infiltrator-contract/src/mini_hud.rs`（`MiniHudReadModel`：真实 traffic/mode/exit/共享开关状态，`status_line()` 由 `SystemToggleState::compact_label` 派生，`next_value()` 是两端唯一的按压规则）；Iced `state.rs::mini_hud_read_model` 从 `diag.traffic`/`runtime.proxy_mode`/`runtime_selected_proxy`/`runtime.system_toggles` 组装，`view/mini_hud.rs` 全卡拖拽 + 真实双通道波形 + 出口胶囊 + 模式芯片 + 两个快捷开关；Bevy `mini_hud_shell.rs::sync_mini_hud_model` 同源组装，`mini_hud.rs::mini_hud_scene` 挂载 `MiniHudRoot` 并首次带出与 Iced 对齐的两个快捷开关（`MiniHudSystemProxyToggle`/`MiniHudTunToggle`，`ButtonDisabled` 由共享 `next_value` 决定），删除硬编码 `"RULE"`/`"系统代理: 开启 · TUN: 开启"`；`ToggleMiniHud` 端到端（Bevy `Ctrl+Alt+M` 派发 + 调色板行 + Iced `on_shell_shortcut`）；测试：Iced `the_mini_hud_read_model_comes_from_live_projections`、Bevy `the_toggle_event_mounts_and_unmounts_the_overlay`/`the_mounted_scene_renders_the_shared_read_model`/`the_hud_quick_switches_dispatch_the_shared_toggle_commands`/`a_pending_hud_quick_switch_dispatches_nothing`。**诚实偏差**：Iced 把宿主主窗口缩到 260x90、移动到持久化坐标并 `Level::AlwaysOnTop`（`mini_hud_window.rs`），仍是主窗口而非第二个独立窗口（Iced 单窗口宿主）；Bevy 为窗口内绝对定位浮层，无独立 OS 窗口、无拖拽、无波形条，故不宣称 `parity-ready` |
...
| `DUAL-15-14` | 双端设计规范与交互质感对齐 | `shared-ready` | 本轮把视觉口径收敛为**可机检的共享 token**：`infiltrator-contract/src/design_tokens.rs`（四皮肤核心调色板 `skin_core(RgbaToken)`：canvas/sidebar/card/card_border/control_bg/ink/ink_dim/accent/on_accent/success/warning/danger + 间距阶梯 4/8/12/16/20/24 + 圆角 16/10 + 发丝线 1px）；Iced `view/theme.rs` 的 `LIGHT/DARK/FOREST/AMOLED` 与 `SP_*`/`R_*` 直接由契约常量构造（编译期绑定，`the_iced_tokens_resolve_the_shared_design_contract` 再逐通道断言）；业务无关的 Bevy widget 层按数值镜像（原本 dark/light 停留在旧 iOS 色板、forest/amoled 的 recessed 填充偏亮，本轮按 Iced 参考值对齐，圆角 12/8 → 16/10），由 `the_widget_palette_mirrors_the_shared_design_tokens`/`the_widget_ladders_mirror_the_shared_contract_numbers` 与守卫 `multimodal-shell-guard.py::check_design_token_mirrors` 三重锁定（守卫直接解析两侧源码数值，文档或注释漂移无法蒙混）；命令面板分类/HUD 开关状态字母/加速键提示同源见 15-03/05。**诚实偏差**：托盘（15-02）、手势（15-07）、无障碍与 IME（15-10/11）、无边框（15-13）仍未双端闭环，1:1 质感对齐未达成，保持 `shared-ready` |
| `DUAL-15-15` | 多模态与外壳架构无头测试矩阵 | `shared-ready` | 新增可机检矩阵 [MULTIMODAL_SHELL_MATRIX.md](MULTIMODAL_SHELL_MATRIX.md)：15 项逐行绑定 `路径::测试名` evidence，守卫会验证每个引用的文件存在且测试真实存在、`parity-ready` 行必须有 Iced+Bevy 双侧 evidence、`planned` 行不得宣称测试；已闭环项（01/04/05/06/08/09/12）双侧无头断言齐备，03/14/15 为 `shared-ready` 并注明偏差来源；02/07/10/11/13 无测试面，矩阵未 100% |
