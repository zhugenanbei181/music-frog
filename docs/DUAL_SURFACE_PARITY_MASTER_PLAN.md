...
| `DUAL-14-08` | DNS 泄漏多源并发交叉探测 | `planned` | **宿主事实缺失（未变，阻塞点已随 14-10 更新）**：本轮 14-10 已落地真实宿主探测端口（`DnsLatencyProbePort` 能向任一配置 Nameserver 发任意随机伪子域并校验应答），但「真实 ISP DNS 泄漏」结论需要一个**受控权威区回显源**（`random.<zone>` 被哪台解析器查询的观测）或多个全球探测源，端口层仍无该事实源；Iced `Message::RunDnsLeakProbe` 的 `country="US"`/`isp="Cloudflare"`/`is_leak_detected=false`/`tested_dns_servers` 仍是硬编码（`update/ui.rs:577`），本批未接线、也未把它包装成结论，保持 `planned`。精确阻塞点：新增「多源泄漏探测」宿主端口 + 受控回显源事实 + 共享快照 + 双端面板 |
