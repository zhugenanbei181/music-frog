...
| 组 15 多模态外壳与极客命令流 | 15 | `in progress (10/15)` | `mini_hud.rs`、`command_palette.rs`、`sidebar.rs`、`tray_status.rs`、`window_chrome.rs`、`a11y.rs` | 2026-09-22 五批展开（见组 15 逐项账目）：01/02/03/04/05/06/08/09/12/13 已双端收口（10/15），10/14/15 为 `shared-ready`，01 由 [RESPONSIVE_PARITY_LEDGER.md](RESPONSIVE_PARITY_LEDGER.md) 权威跟踪、矩阵见 [MULTIMODAL_SHELL_MATRIX.md](MULTIMODAL_SHELL_MATRIX.md)，07/11 `planned` |
...
| `DUAL-10-12` | 扩展脚本导出与社区分享 | `shared-ready` | 共享 `ExtensionPackage` + SHA-256 校验（`script_engine.rs:89`）与 `export_extension`/`import_extension`（`script_application.rs`）；矩阵 `check_extension_round_trip` 实测导出→导入→校验和；**诚实边界**：无逐端导出 UI（模板要求 `.js` 文件，当前为 JSON 包），故不记 `parity-ready` |
...
> 经 surface 读模型下发，不在 Bevy 内执行）；10-09 仍为非三栏、10-12 仍无 `.js`
> 导出 UI，均未收口。守卫更新为固化上述标记（含 `ScriptEngineKind`、
...
| `DUAL-15-10` | AccessKit 无障碍语义全覆盖 | `shared-ready` | 共享语法 `infiltrator-contract/src/a11y.rs`：`A11yRole`（Window/Header/Region/Navigation/Button/Switch/Status/Dialog/LiveRegion/Text）与 `ShellA11yNode::ALL` 18 行（角色 + i18n `label_key` + 裸中文 `label_zh` + `is_switch` 状态规则），`shell_a11y_specs()` 给两端同一张表。Bevy 发布真实 AccessKit：`bevy-ui/src/a11y.rs::{accesskit_role,semantic_node,switch_node,value_node}`，`shell_scene.rs` 挂载窗口/标题/内容区/导航/状态圆点/模式段/主题/系统代理与 TUN 开关（携带 `set_toggled` 实时状态）/速率读数，`chrome.rs` 三个窗口控件、`mini_hud.rs` 卡片与两个快捷开关（`switch_node`）、`command_palette.rs` 对话框、`toast.rs::sync_toast_semantics` 通知 live region；`the_mounted_shell_publishes_every_shared_semantic_row` 遍历 `ShellA11yNode::ALL` 断言每个角色+标签都已挂载（开关还必须有状态）。Iced 0.14 无 AccessKit 接入，**不伪造**：`accessibility.rs` 把同一 `label_key()` 经本地化解析（`a11y_label`/`a11y_role`）并作为真实可见 tooltip 挂到状态圆点、速率读数、Mini HUD 卡片与快捷开关、三个窗口控件（`view/sidebar.rs`/`view/mini_hud.rs`/`view/chrome.rs`）；测试 `every_shared_semantic_node_resolves_a_localized_label_and_role`/`the_shell_carries_the_shared_labels_as_visible_tooltips`。守卫新增 `check_a11y_label_coverage`（每个 label key 在 zh/en 表都存在）与 `check_capability_honesty`（Iced 代码不得出现 `accesskit`）。**缩小的声明范围**：覆盖对象是外壳节点 18 行，不含每个页面内的控件；Iced 无法向 OS 上报角色（无 AccessKit），其证据是共享标签的真实可见消费而非语义树 |
| `DUAL-15-11` | IME 中文输入法候选框定位 | `planned` | 两端均未接线（无组合事件消费、无候选框定位、无共享契约）。宿主事实澄清（批次 E 复核）：Bevy 0.19 暴露真实 IME 路径（`Window.ime_enabled`/`Window.ime_position` → `winit::Window::set_ime_cursor_area`，事件 `bevy_window::event::Ime`），但 Bevy widget 层 `text_input.rs` 只在文档注释里提到 Ime、无实际处理；Iced 0.14 的 `input_method` 事件已贯通（iced_winit 转换 `Ime::{Preedit,Commit}`），但**不暴露** `set_ime_cursor_area` 等价 API，候选框坐标无法由 Iced 侧控制。本批不伪造：文本输入与拼音检索仍只走键盘字符路径，保持 `planned` |
