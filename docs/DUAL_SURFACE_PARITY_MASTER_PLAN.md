...
| 组 14 DNS 工作台与泄漏探活 | 15 | `in progress (13/15)` | `dns.rs`（两端） | 2026-09-22 起逐项展开（见组 14 逐项账目）：01/02/03/04/05/06/07/10/11/12/13/14/15 已双端收口（13/15 `parity-ready`），08 `shared-ready`（真实回显适配器已注入 desktop runtime/boot，但本机无受控权威区，两端如实显示 typed unsupported）、09 `planned`（需要浏览器/STUN 宿主端口） |
