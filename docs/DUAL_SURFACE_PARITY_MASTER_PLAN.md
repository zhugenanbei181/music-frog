...
| 组 10 脚本沙箱与多级 Mixin | 15 | `in progress (14/15)` | `script_console.rs`、`profiles_script.rs`、`mixin_studio.rs`、`script_sandbox.rs`、`script_export.rs` | 2026-09-23 批次 B/C 收口（见组 10 逐项账目）：02-08、10-15 双端 `parity-ready`（14/15），01 `planned`（无真实 QuickJS 引擎，绝不冒充 JS 执行），09 三栏式 Mixin 编辑器与 12 逐端导出均于批次 C 收口为 `parity-ready` |
...
| 项 | 任务 | 状态 | 证据 |
| :--- | :--- | :--- | :--- |
| `DUAL-10-01` | QuickJS 嵌入式轻量执行沙箱 | `planned` | **无真实 QuickJS 引擎**：`infiltrator-domain/src/script_engine.rs:40` 的 `execute_transform` 经 `script_engine_directives.rs:13` 用正则识别已知指令后改写 YAML AST；`script_engine_runtime.rs:72` 甚至以字符串匹配拒绝 `while(true)`。这是诚实的指令 DSL，不是 JS 解释器，故不宣称沙箱执行；矩阵 `DUAL-10-01` 行明示未覆盖。**本项是产品/依赖决策，不是实现缺口**：可选路线与阻塞条件见批次 C 说明（引入 `rquickjs`/`boa_engine` 需先定依赖准入、FFI 交叉编译、脚本 ABI 与内核回调边界），在决策落地前不添加任何 JS 引擎依赖（守卫反向断言禁止出现 `rquickjs`/`boa_engine`/`quickjs::`） |
...
| `DUAL-15-07` | 移动端触控手势引擎 | `planned` | 仅 widget 层纯实现 + widget 自测：`infiltrator-bevy-widgets/src/gesture.rs`（`GestureRecognizer:61`、`PullToRefreshState:156`、`SwipeToActionItem:203`、`SafeAreaInsets:13`）与 `tests/headless/advanced_ecosystem_tests.rs`；`bevy-ui`/Android host 零引用，无共享契约，保持 `planned`。**批次 F 完善阻断事实**：`TouchInput` 在壳层只以能力位出现（`infiltrator-bevy-widgets/src/abi.rs` 的 `WidgetCapability::TouchInput`），全仓没有任何 `MessageReader<TouchInput>` 消费者，也没有 `SafeAreaInsets` 消费者；`crates/infiltrator-bevy-ui`、`crates/infiltrator-android`（uniffi/FFI 宿主）、`crates/infiltrator-ios` 对 `gesture` 零引用，移动宿主不组合 Bevy 外壳——既无共享语义契约也无可断言的宿主接缝，不把 widget 自测包装成宿主机能 |
