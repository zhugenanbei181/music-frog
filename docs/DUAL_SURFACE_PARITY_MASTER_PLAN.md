...
| 组 14 DNS 工作台与泄漏探活 | 15 | `in progress` | `dns.rs`（两端） | 2026-09-22 起逐项展开（见组 14 逐项账目）：01/02/03/04/05/06/07/10/11/12/13/14/15 已双端收口（13/15 `parity-ready`），08/09 `planned`（宿主事实缺失：多源泄漏结论需要受控权威区回显源、WebRTC 需要浏览器/STUN 宿主端口） |
| 组 15 多模态外壳与极客命令流 | 15 | `in progress (13/15)` | `mini_hud.rs`、`command_palette.rs`、`sidebar.rs`、`tray_status.rs`、`window_chrome.rs`、`a11y.rs`、`ime.rs`、`design_tokens.rs` | 2026-09-23 七批展开（见组 15 逐项账目）：01/02/03/04/05/06/08/09/11/12/13/14/15 已双端收口（13/15），10 为 `shared-ready`（Iced 无 AccessKit 接入的能力边界，标签/提示已真实消费并机检），01 由 [RESPONSIVE_PARITY_LEDGER.md](RESPONSIVE_PARITY_LEDGER.md) 权威跟踪、矩阵见 [MULTIMODAL_SHELL_MATRIX.md](MULTIMODAL_SHELL_MATRIX.md)，07 `planned` |
...
> **诚实边界**：本产品未内置受控回显权威区，桌面默认源列表为空，双端如实显示 typed
> `Unsupported`（真实泄漏结论不在无配置时给出）；因此保持 `shared-ready` 而非
> `parity-ready`，闭环只差宿主配置一个 `authority`。14-09 仍缺浏览器/STUN 宿主事实源。
...
| `DUAL-15-07` | 移动端触控手势引擎 | `planned` | 仅 widget 层纯实现 + widget 自测：`infiltrator-bevy-widgets/src/gesture.rs`（`GestureRecognizer:61`、`PullToRefreshState:156`、`SwipeToActionItem:203`、`SafeAreaInsets:13`）与 `tests/headless/advanced_ecosystem_tests.rs`；`bevy-ui`/Android host 零引用，无共享契约，保持 `planned`。**批次 F 完善阻断事实**：`TouchInput` 在壳层只以能力位出现（`infiltrator-bevy-widgets/src/abi.rs` 的 `WidgetCapability::TouchInput`），全仓没有任何 `MessageReader<TouchInput>` 消费者，也没有 `SafeAreaInsets` 消费者；`crates/infiltrator-bevy-ui`、`crates/infiltrator-android`（uniffi/FFI 宿主）、`crates/infiltrator-ios` 对 `gesture` 零引用，移动宿主不组合 Bevy 外壳——既无共享语义契约也无可断言的宿主接缝，不把 widget 自测包装成宿主机能。**批次 G 复核（2026-09-23）**：`MessageReader<*TouchInput>`/`EventReader<*TouchInput>` 全仓仍为 0 处；`infiltrator-android`/`infiltrator-ios` 的 Cargo.toml 完全不依赖 `bevy`/`infiltrator-bevy-ui`/`infiltrator-bevy-widgets`，`crates/*/Cargo.toml` 中没有任何宿主依赖 `infiltrator-bevy-ui`（只有它自己），即「移动宿主 + Bevy 外壳」这一 Seam 在依赖图上不存在；唯一 `gesture` 命中是 `bevy-ui/src/pages/overview.rs` 注释里的 Bevy `Activate` 指针事件，与触控手势引擎无关——本项保持 `planned`，不伪造手势链路 |
