...
- 已完成：配置切换/导入/保存/内核切换增加操作锁定与重启进度等待反馈。
- 已完成：Windows 计划任务/提权等命令统一无窗口启动，减少启动时 cmd 闪烁。
- 已完成：订阅导入与激活解耦，导入快速返回，重启后台执行并给出明确提示。
- 已完成：配置管理界面改为 Vue 3 + TS + Tailwind，分区布局与结果提示更清晰。
...
- 已完成：撤销 MSI 安装界面定制，恢复默认安装流程，避免定制失败阻塞打包。
