#!/usr/bin/env bash
# Capture the Infiltrator iced-frontend demo screenshot matrix in one fully
# backgrounded compositor stack (native iced render evidence — see
...
# docs/screenshots/iced/. The operator's Wayland session is never contacted:
...
#
# Usage:
#   bash scripts/capture-iced-matrix.sh                  # full matrix
#   bash scripts/capture-iced-matrix.sh proxies-dark     # one scenario
#   INFILTRATOR_CAPTURE_SCENARIOS=a,b bash scripts/capture-iced-matrix.sh
#
...
# Runtime check (niri msg -j windows): iced/winit maps the demo window with an
# EMPTY app_id, so app_id cannot be the identity key; the window title is the
# stable observed identifier ("MusicFrog Infiltrator" in every locale). The
# spawned PID stays the authoritative binding either way.
APP_TITLE="MusicFrog Infiltrator"
...
RUNTIME_DIR="$(mktemp -d /tmp/infiltrator-iced-niri.XXXXXX)"
...
cleanup() {
  ...
  # dirs. Run evidence under target/iced-evidence/ is always preserved.
...
// INFILTRATOR_WINDOW_SIZE instead of being tiled/resized by the compositor.
// Both matches are kept because iced/winit has been observed to map windows
// with either the binary-name app_id or an empty one depending on version;
// the title is stable across locales ("MusicFrog Infiltrator").
...
layout {
    ...
    // off so the PNG is exactly the app's requested INFILTRATOR_WINDOW_SIZE.
    shadow {
        off
    }
}
window-rule {
    match app-id="$APP_ID"
    match title="^MusicFrog Infiltrator\$"
    open-floating true
}
KDL
...
start_capture_host() {
  KWIN_RUNTIME="$(mktemp -d /tmp/infiltrator-iced-kwin.XXXXXX)"
...
capture_one() {
  ...
  [ "$marker_page" = "mixin" ] && marker_page="editor"
  [ "$marker_page" = "filter" ] && marker_page="editor"
  [ "$marker_page" = "rules-providers" ] && marker_page="rules"
  ...
  local published="docs/screenshots/iced/$name.png"
  ...
  # late-session launch flakiness. Each attempt tears the previous app down
  # and starts clean; the manifest row is written exactly once from the final
  ...
  while [ "$attempt" -le 2 ]; do
    ...
    printf '  start %-22s page=%-8s skin=%-5s window=%s\n' \
    ...
    XDG_RUNTIME_DIR="$RUNTIME_DIR" WAYLAND_DISPLAY="$NIRI_SOCK" DISPLAY= \
      ...
      setsid "$APP" --demo >"$log" 2>&1 &
    APP_PID=$!
    APP_PGID="$(process_group "$APP_PID")"

    if [ "$APP_PGID" = "$APP_PID" ]; then
      # Window discovery first: bind to the exact spawned PID plus the observed
      # identity (title; app_id is empty for iced/winit but kept as an
      # alternative match), so the screenshot can never attach to a leftover
      # or foreign window. windows.json is kept per scenario as receipt.
    ...
    if [ "$action_status" = ok ]; then
      ...
      # PNG receipt: readable IHDR and exactly the requested window size
      # (nested output scale=1 makes logical == physical pixels).
      local wait
    ...
    if ! kill -0 "$APP_PID" 2>/dev/null; then
      break # a crashed app will not do better on a retry
    fi
    attempt=$((attempt + 1))
  done
  ...
  if [ "$action_status" != ok ] && [ "$action_status" != "failed-dims"* ]; then
    if [ "$window_ready" -eq 0 ]; then
      ...
      action_status="failed-marker (no CAPTURE_READY within ${MARKER_TIMEOUT_ITERS}x0.25s)"
    fi
  fi
